...
class JNMF():
	...
	def initializeWH(self):
		self.W = pd.DataFrame(np.random.randint(0,100,size=(self.X.shape[0], self.K)), index=list(self.X.index))
		self.Hx = pd.DataFrame(np.random.randint(0,100,size=(self.K, self.X.shape[1])), columns=list(self.X))
		self.Hy = pd.DataFrame(np.random.randint(0,100,size=(self.K, self.Y.shape[1])), columns=list(self.Y))
		self.Hz = pd.DataFrame(np.random.randint(0,100,size=(self.K, self.Z.shape[1])), columns=list(self.Z))
