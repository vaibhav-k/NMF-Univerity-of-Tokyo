...
class JConsensus:
    def __init__(self, X, Y, Z):
        self.cmW = pd.DataFrame(
            np.zeros((X.shape[0], X.shape[0])), index=X.index, columns=X.index
        )
        self.cmHx = pd.DataFrame(
            np.zeros((X.shape[1], X.shape[1])), index=X.columns, columns=X.columns
        )
        self.cmHy = pd.DataFrame(
            np.zeros((Y.shape[1], Y.shape[1])), index=Y.columns, columns=Y.columns
        )
        self.cmHz = pd.DataFrame(
            np.zeros((Z.shape[1], Z.shape[1])), index=Z.columns, columns=Z.columns
        )
        self.cmNum = 0
