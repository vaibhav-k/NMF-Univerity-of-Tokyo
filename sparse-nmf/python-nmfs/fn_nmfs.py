...
class SparseNMF:
    ...
    def update(self):
        self.H = np.multiply(
            self.H,
            np.divide(
                np.dot(self.W.T, np.multiply(self.X, np.power(self.A, self.beta - 2))),
        ...
        self.W = np.multiply(
            self.W,
            np.divide(
                np.dot(np.multiply(np.power(self.A, self.beta - 2), self.X), self.H.T),
    ...
    def calc_error(self):
        self.error = np.mean(np.mean(np.abs(self.X - self.A))) / np.mean(
            np.mean(self.X)
        )
