...
class NMF:
    ...
    def initializeWH(self):
        self.W = pd.DataFrame(
            np.random.randint(0, 100, size=(self.X.shape[0], self.K)), dtype=float
        )
        self.H = pd.DataFrame(
            np.random.randint(0, 100, size=(self.K, self.X.shape[1])), dtype=float
        )
        ...
        for run in range(self.iterations):
            ...
            self.H = np.multiply(
                ...
                np.divide(
                    ...
                    np.dot(
                        self.W.T,
                        np.multiply(self.mX, np.dot(self.W, self.H) + self.eps),
                    ),
                ),
            )
            self.W = np.multiply(
                self.W,
                np.divide(
                    np.dot(np.multiply(self.mX, self.X), self.H.T),
                    (
                        np.dot(np.multiply(self.mX, np.dot(self.W, self.H)), self.H.T)
                        + self.eps
                    ),
                ),
            )
