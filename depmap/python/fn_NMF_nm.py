...
class NMF():
	...
	def initializeWH(self):
		self.W = pd.DataFrame(np.random.randint(0,100,size=(self.X.shape[0], self.K)), dtype=float, index=list(self.X.index))
		self.H = pd.DataFrame(np.random.randint(0,100,size=(self.K, self.X.shape[1])), dtype=float, columns=list(self.X))
