...
class NMF():
	...
	def update(self):
		...
		self.W = np.multiply(self.W, np.divide(np.dot(np.multiply(np.c_[self.mX], np.c_[self.X]), np.transpose(np.c_[self.H])), (np.dot(np.multiply(np.c_[self.mX], np.dot(self.W, np.c_[self.H])), np.transpose(np.c_[self.H]))+self.eps)))
